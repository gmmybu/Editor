...
BOOL cActionTemplate::Load(const char *Filename)
{
	FILE *fp;
	char Text[2048];
...
BOOL cActionTemplate::ExpandDefaultActionText(char *Buffer, int Size, sAction *Action)
{
	long i, Pos, Num;
	char Text[256];
	...
	if(!Action->NumEntries) 
	{
		strcpy_s(Buffer, Size, Action->Text);
		return TRUE;
	}
	...
	for(i=0;i<(long)strlen(Action->Text);i++) 
	{
		...
		if(Action->Text[i] == '~') 
		{
			...
			{
				sprintf_s(Text, "(*%lu*)", Action->Entries[Num].lMin);
			...
			{
				sprintf_s(Text, "(*%lf*)", Action->Entries[Num].fMin);
				memcpy(&Buffer[Pos], Text, strlen(Text));
				Pos += strlen(Text);
			} else
...
BOOL cActionTemplate::ExpandActionText(char *Buffer, int Size, sScript *Script)
{
	long i, j, Pos, Num;
	char Text[256];
	sAction *ActionPtr;
	...
	{
		strcpy_s(Buffer, Size, ActionPtr->Text);
		return TRUE;
	}
	...
	for(i=0;i<(long)strlen(ActionPtr->Text);i++) 
	{
		...
		if(ActionPtr->Text[i] == '~') 
		{
			...
			if(ActionPtr->Entries[Num].Type == _TEXT) 
			{
				...
				{
					for(j=0;j<Script->Entries[Num].Length;j++) 
					{
			...
			{
				sprintf_s(Text, "(*%lu*)", Script->Entries[Num].lValue);
				memcpy(&Buffer[Pos], Text, strlen(Text));
				Pos += strlen(Text);
			} else
			...
			{
				sprintf_s(Text, "(*%lf*)", Script->Entries[Num].fValue);
				memcpy(&Buffer[Pos], Text, strlen(Text));
				Pos += strlen(Text);
			} else
			...
			if(ActionPtr->Entries[Num].Type == _CHOICE) 
			{
				...
				char *Tmp = ActionPtr->Entries[Num].Choices[Script->Entries[Num].Selection];
				memcpy(&Buffer[Pos], Tmp, strlen(Tmp));
